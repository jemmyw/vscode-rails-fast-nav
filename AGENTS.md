...
- Keep `vscode-extension-tester` on 8.x+. Old 4.x only fetched Intel `mac64` VS Code and chromedriver from the retired Google bucket, which breaks on Apple Silicon and Chrome >= 115. 8.x pulls `darwin-universal` VS Code + Chrome-for-Testing chromedriver. `skipLibCheck` is on because extest's selenium/ws type deps need newer lib globals.
